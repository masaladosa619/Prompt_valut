...
const ShapeGrid = ({
  ...
  useEffect(() => {
    ...
    const updateAnimation = () => {
      ...
      switch (direction) {
        case 'right':
          gridOffset.current.x = (gridOffset.current.x - eff + wrapX) % wrapX;
          break;
        case 'left':
          gridOffset.current.x = (gridOffset.current.x + eff + wrapX) % wrapX;
          break;
        case 'up':
          gridOffset.current.y = (gridOffset.current.y + eff + wrapY) % wrapY;
          break;
        case 'down':
          gridOffset.current.y = (gridOffset.current.y - eff + wrapY) % wrapY;
          break;
    ...
    const updateCellOpacities = () => {
      ...
      // ensure all target keys exist in map
      targets.forEach((_, k) => {
    ...
    const handleMouseMove = e => {
      ...
      if (isHex) {
        ...
        const ax = mx - offsetX;
        const ay = my - offsetY;
        const col = Math.round(ax / halfW);
        const row = Math.floor(ay / squareSize);
        if (!hoveredSquare.current || hoveredSquare.current.x !== col || hoveredSquare.current.y !== row) {
          if (hoveredSquare.current && hoverTrailAmount > 0) {
            trailCells.current.unshift({ ...hoveredSquare.current });
            if (trailCells.current.length > hoverTrailAmount) trailCells.current.length = hoverTrailAmount;
        ...
        const ax = mx - offsetX;
        const ay = my - offsetY;
        const col = Math.round(ax / squareSize);
        const row = Math.round(ay / squareSize);
        if (!hoveredSquare.current || hoveredSquare.current.x !== col || hoveredSquare.current.y !== row) {
          if (hoveredSquare.current && hoverTrailAmount > 0) {
            trailCells.current.unshift({ ...hoveredSquare.current });
            if (trailCells.current.length > hoverTrailAmount) trailCells.current.length = hoverTrailAmount;
        ...
        const ax = mx - offsetX;
        const ay = my - offsetY;
        const col = Math.floor(ax / squareSize);
        const row = Math.floor(ay / squareSize);
        if (!hoveredSquare.current || hoveredSquare.current.x !== col || hoveredSquare.current.y !== row) {
          if (hoveredSquare.current && hoverTrailAmount > 0) {
            trailCells.current.unshift({ ...hoveredSquare.current });
